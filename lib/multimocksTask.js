...
var MultimocksTask = function (logger) {
  log = logger;
}
/**
 ...
 */
MultimocksTask.prototype.writeScenarioData = function (files) {
  files.forEach(function (taskConfig) {
    ...
    log('mockSrc', mockSrc);
    log('dest', taskConfig.dest);
    log('template', taskConfig.template);
    log('multipleFiles', taskConfig.multipleFiles);
    log('plugins', taskConfig.plugins);

    // read all scenario data from manifest/JSON files
    var scenarioData = readScenarioData(taskConfig, mockSrc);

    log('scenarioData', scenarioData);
